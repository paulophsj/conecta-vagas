...
export const enviarCandidatura = async (id) => {
    try {
        const response = await Fetch(`http://localhost:8080/api/candidatura/${id}`, {
            method: "POST"
        })
...
export const checkAllCandidaturas = async () => {
    try {
        const response = await Fetch(`http://localhost:8080/api/candidatura`, {
            method: "GET"
        })
        const data = await response.json()
...
export const checkAllCandidaturasByIdVaga = async (id) => {
    try {
        const response = await Fetch(`http://localhost:8080/api/candidatura/recrutador/${id}`, {
            method: "GET"
        })
        const data = await response.json()
